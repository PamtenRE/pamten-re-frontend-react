...
export default function Navbar() {
  ...
  const handleDashboardClick = () => {
    if (user?.role === "recruiter") {
      ...
      // Handle candidate home route (dashboard removed)
  ...
  const isOnDashboard =
    pathname.includes("/recruiter/") || pathname.includes("/candidate/");

  return (
    <nav className="w-full px-6 py-4 flex justify-between items-center bg-white/80 dark:bg-black/70 backdrop-blur-md border-b border-gray-200/50 dark:border-white/10 fixed top-0 left-0 right-0 z-[9999]">
      <div
      ...
      {/* Search Bar - Only show on candidate pages */}
      {isAuthenticated && pathname.includes("/candidate/") && (
        <SearchBar className="flex-1 max-w-2xl mx-4" />
      )}
